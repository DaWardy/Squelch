...
| 14.2 | Reverse / fast-forward playback | ✅ **DONE** (2026-07-18, `ba7caa6`) — `IQPlayer` reverse playback (backward chunks, time-reversed, stops at start) + live `set_speed` (to 8×) / `set_reverse`; "◀ Rev" toggle + Speed combo in the transport. 4 tests | YES | P1 |
...
| 14.4 | **Multiple receivers** (N parallel demodulators, matrix) | 🟡 **engine DONE** (2026-07-18, `1f46761`) — `core/demod.py`: the missing IQ→audio DSP (AM/FM/NBFM/WFM/USB/LSB/CW: tune→lowpass→decimate→demodulate) + `MultiVFO` (N `VFOChannel` over one IQ stream, in-band gated). 15 tests (each mode recovers its exact tone; channel separation). **Remaining: the matrix UI + real audio output** (sounddevice) — launch session | engine YES | **P1** |
| 14.5 | Auto-notch + manual notch filters | ✅ **core DONE** (2026-07-18, `2ad8d15`) — `core/audio_dsp.py`: `notch()` (manual band-stop) + `auto_notch()` (detect + remove strong het tones via FFT prominence over an edge-corrected baseline; leaves broadband alone). 9 tests. Wire into the audio path when it lands | YES | P2 |
...
| 14.7 | Audio (demod) recording → WAV | ✅ **DONE** (2026-07-18, `6b44a44`) — `core/audio_record.py` (`write_wav` + `AudioRecorder`: demod each IQ frame → optional auto-notch → mono 16-bit WAV) + a "🎧 Rec Audio" toolbar toggle fed from `_on_samples`. **First way to actually HEAR a signal** (open the .wav in any player) — no audio device, no dongle (Simulated source / played capture). 9 tests incl. recovered-tone + Qt wiring | YES | P2 |
