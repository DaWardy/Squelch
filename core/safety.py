...
class SafetyManager:
    ...
    def can_transmit(self) -> bool:
        ...
        Returns False in Demo mode (C-06, Elena) so a classroom lecture can
        never accidentally transmit, and only True in safe states (C-08,
        Hank — never key unexpectedly).

        NOTE: Guest Operator mode does NOT block transmit. A guest operator
        or student is a licensed/supervised op who gets on the air normally;
        that is handled separately (guest callsign + contact script)."""
    ...
    def set_demo_mode(self, enabled: bool):
        ...
        showing the software without any risk of keying the rig."""
        self._demo_mode = bool(enabled)

    # Backwards-compat shims (the old name meant the TX-block, i.e. demo)
    def set_guest_mode(self, enabled: bool):  # deprecated alias
