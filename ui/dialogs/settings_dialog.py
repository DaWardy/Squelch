...
class SettingsDialog(QDialog):
    ...
    def _apply_live(self):
        ...
        try:
            ...
            # 1. Update global font (affects new widgets)
            f = QFont()
            f.setPointSize(fs)
            app.setFont(f)

            # 2. Apply themed stylesheet with font size
            app.setStyleSheet(get_stylesheet(theme, fs))

            # 3. Force ALL top-level windows to repoll font
            #    This is needed because inline stylesheets
            #    override QApplication stylesheet
            for window in app.topLevelWidgets():
                self._set_font_recursive(window, f)
    ...
    def _set_font_recursive(self, widget, font):
        """
        Recursively update font on all child QWidgets.
        Also rewrites inline stylesheet font-size values
        to match the selected point size — necessary because
        inline stylesheets override QApplication stylesheet.
        """
        import re
        ...
        pt = font.pointSize()
        # Map the target point size to a px equivalent
        # (standard 96dpi: 1pt ≈ 1.333px)
        target_px = round(pt * 1.333)

        # Scale reference: relative to 11pt base (≈15px)
        base_px = 15
        scale   = target_px / base_px

        def _patch_style(ss: str) -> str:
            """Scale all font-size:Npx in a stylesheet."""
            def _repl(m):
                orig = int(m.group(1))
                new_val = max(9, round(orig * scale))
                return f"font-size:{new_val}px"
            return re.sub(r'font-size:(\d+)px', _repl, ss)

        def _apply(w):
            w.setFont(font)
            ss = w.styleSheet()
            if ss and "font-size:" in ss:
                patched = _patch_style(ss)
                if patched != ss:
