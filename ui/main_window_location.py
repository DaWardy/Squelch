...
class _MainWindowLocationMixin:
    ...
    def _apply_grid_result(self, grid: str, display: str = "",
                           lat: float = 0.0, lon: float = 0.0):
        """Apply a resolved grid to config and notify all tabs."""
        if not grid:
            return
        grid = grid.upper()
        self._grid_lbl.setText(grid)
    ...
    def _run_location_search(self, q: str):
        """Thread target: search for *q*, emit _location_found or _location_failed."""
        from core.location import _latlon_to_grid
        try:
            ...
            if loc and loc.is_valid:
                ...
                if grid:
                    ...
                    self._location_found.emit(
                        grid, disp,
                        float(getattr(loc, "lat", 0.0) or 0.0),
                        float(getattr(loc, "lon", 0.0) or 0.0))
                else:
                    self._location_failed.emit("Not found — try grid square")
            else:
